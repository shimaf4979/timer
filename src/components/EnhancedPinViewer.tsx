...
const EnhancedPinViewer: React.FC<EnhancedPinViewerProps> = ({
    ...
    const updatePinPosition = () => {
      ...
      try {
        ...
        const floorContainer =
          containerRef.current.closest('.floor-container') || containerRef.current;
  ...
  }, [pin.x_position, pin.y_position, containerRef]);
  ...
  return (
    <>
      <button
        ...
        className={`absolute transform -translate-x-1/2 -translate-y-1/2 ${fixedTooltip || isSelected ? 'selected-pin pin-hover-animation' : ''}`}
        style={{
      ...
      >
        <div className="w-6 h-12 relative flex flex-col items-center group">
          {/* ピンのヘッド部分 */}
          <div
            className={`w-6 h-6 ${getPinColor()} rounded-full flex items-center justify-center text-white 
              ...
              className={`h-4 w-2 ${
                fixedTooltip || isSelected
                  ...
                  : isPublicEdit && currentEditorId === pin.editor_id
                    ? 'bg-green-600'
                    : 'bg-red-600'
              } z-0 transform-gpu rounded-b-none mt-4`}
            ></div>
              ...
              className={`h-3 w-2 ${
                fixedTooltip || isSelected
                  ...
                  : isPublicEdit && currentEditorId === pin.editor_id
                    ? 'bg-green-700'
                    : 'bg-red-700'
              } clip-path-triangle z-0 transform-gpu`}
            ></div>
          </div>
